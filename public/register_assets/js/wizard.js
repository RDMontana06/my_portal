...
class Wizard {
    constructor(args) {
        let opts = {
            wz_class:
                args != undefined && args.hasOwnProperty("wz_class")
                    ? args.wz_class
                    : ".wizard",
            wz_nav:
                args != undefined && args.hasOwnProperty("wz_nav")
                    ? args.wz_nav
                    : ".wizard-nav",
            wz_ori:
                args != undefined && args.hasOwnProperty("wz_ori")
                    ? args.wz_ori
                    : ".horizontal",
            wz_nav_style:
                args != undefined && args.hasOwnProperty("wz_nav_style")
                    ? args.wz_nav_style
                    : "dots",
            wz_content:
                args != undefined && args.hasOwnProperty("wz_content")
                    ? args.wz_content
                    : ".wizard-content",
            wz_buttons:
                args != undefined && args.hasOwnProperty("wz_buttons")
                    ? args.wz_buttons
                    : ".wizard-buttons",
            wz_button:
                args != undefined && args.hasOwnProperty("wz_button")
                    ? args.wz_button
                    : ".wizard-btn",
            wz_button_style:
                args != undefined && args.hasOwnProperty("wz_button_style")
                    ? args.wz_button_style
                    : ".btn",
            wz_step:
                args != undefined && args.hasOwnProperty("wz_step")
                    ? args.wz_step
                    : ".wizard-step",
            wz_form:
                args != undefined && args.hasOwnProperty("wz_form")
                    ? args.wz_form
                    : ".wizard-form",
            wz_next:
                args != undefined && args.hasOwnProperty("wz_next")
                    ? args.wz_next
                    : ".next",
            wz_prev:
                args != undefined && args.hasOwnProperty("wz_prev")
                    ? args.wz_prev
                    : ".prev",
            wz_finish:
                args != undefined && args.hasOwnProperty("wz_finish")
                    ? args.wz_prev
                    : ".finish",

            current_step:
                args != undefined && args.hasOwnProperty("current_step")
                    ? args.current_step
                    : 0,
            steps:
                args != undefined && args.hasOwnProperty("steps")
                    ? args.steps
                    : 0,
            navigation:
                args != undefined && args.hasOwnProperty("navigation")
                    ? args.navigation
                    : "all",
            buttons:
                args != undefined && args.hasOwnProperty("buttons")
                    ? args.buttons
                    : true,
            next:
                args != undefined && args.hasOwnProperty("next")
                    ? args.next
                    : "Next",
            prev:
                args != undefined && args.hasOwnProperty("prev")
                    ? args.prev
                    : "Prev",
            finish:
                args != undefined && args.hasOwnProperty("finish")
                    ? args.finish
                    : "Submit",
        };
    ...
    init() {
        try {
            let wz = $_.exists($_.getSelector(this.wz_class))
                ? $_.getSelector(this.wz_class)
                : $_.throwException(i18n.empty_wz);
            ...
            let wz_nav = $_.exists($_.getSelector(this.wz_nav, wz))
                ? $_.getSelector(this.wz_nav, wz)
                : $_.throwException(i18n.empty_nav);

            let wz_content = $_.exists($_.getSelector(this.wz_content, wz))
                ? $_.getSelector(this.wz_content, wz)
                : $_.throwException(i18n.empty_content);

            var wz_nav_steps = $_.getSelectorAll(this.wz_step, wz_nav);
            var wz_nav_steps_length =
                wz_nav_steps.length > 0
                    ? wz_nav_steps.length
                    : $_.throwException(i18n.empty_nav);

            var wz_content_steps = $_.getSelectorAll(this.wz_step, wz_content);
            var wz_content_steps_length =
                wz_content_steps.length > 0
                    ? wz_content_steps.length
                    : $_.throwException(i18n.empty_content);
    ...
    update2Form() {
        ...
        if (wz_content.tagName !== "FORM") {
            ...
            $form.setAttribute(
                "class",
                wz_content_class + " " + this.wz_form.replace(".", "")
            );
    ...
    setNav() {
        ...
        if ($_.exists(wz_nav) === false) {
            ...
            for (var i = 0; i < steps_length; i++) {
                ...
                let title = steps[i].hasAttribute("data-title")
                    ? steps[i].getAttribute("data-title")
                    : `${i18n.title} ${i}`;
    ...
    setButtons() {
        ...
        if ($_.exists(wz_btns) === false) {
            ...
            finish.classList.add(this.wz_finish.replace(".", ""));
            // if (this.finish) {
            //     finish.classList.remove("wizard-btn");
            // }
            finish.type = "submit";
    ...
    onClick(e) {
        ...
        let step =
            $_.str2bool($this.getAttribute("data-step")) !== false
                ? parseInt($this.getAttribute("data-step"))
                : this.getCurrentStep();
    ...
    setStep(step) {
        ...
        let check_content = $_.getSelector(
            `${this.wz_step}[data-step="${step}"]`,
            content
        );

        if ($_.exists(check_content) === false) {
            let content_length =
                $_.getSelectorAll(this.wz_step, content).length - 1;
...
var $_ = {
    ...
    formValidator: function (formData) {
        ...
        for (let e of formData) {
            if (
                $_.hasClass(e, "required") ||
                $_.exists(e.getAttribute("required"))
            ) {
                var check = false;
                switch (e.tagName) {
                    case "INPUT":
                ...
                if (check === false) {
                    error = true;
                    console.log("Value of E:" + e.getAttribute("type"));
